...
# Empty = every registered dataset (the R1 behaviour). A name = that one dataset, which is what a
...
# `onstart` field -- the server calls it `args` -- and that field is capped at 16384 bytes. The
# bootstrap alone is ~16.3 KB, so pasting it verbatim made the create call fail with
#   400/3471: len(args) > 16384
# The comment that used to live here asserted "length is not a concern" because --onstart takes
# a FILENAME rather than one argument. That belief was never tested; the first real run disproved
# it. gzip+base64 takes ~16.3 KB to ~9 KB, which fits with room to spare.
BOOTSTRAP_B64="$(gzip -9c "$REPO_DIR/scripts/gpu_helpers/bootstrap_pilot.sh" | base64 | tr -d '\n')"

{
    ...
    printf 'export TFM_BOOTSTRAP_B64=%q\n' "$BOOTSTRAP_B64"
    echo 'echo "$TFM_BOOTSTRAP_B64" | base64 -d | gzip -dc > /tmp/tfm-bootstrap.sh'

# Fail HERE -- locally and for free -- not as a 400 from the API after an offer was picked.
ONSTART_BYTES="$(wc -c < "$ONSTART_FILE" | tr -d ' ')"
if [ "$ONSTART_BYTES" -gt 16000 ]; then
    echo "FATAL: the generated onstart script is $ONSTART_BYTES bytes." >&2
    echo "The API caps that field at 16384 (400/3471 \"len(args) > 16384\")." >&2
    echo "Shrink the inlined payload before re-running." >&2
    exit 1
fi
    echo 'bash /tmp/tfm-bootstrap.sh'
} > "$ONSTART_FILE"
