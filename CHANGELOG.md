...
- docs: zero-inflation study — `docs/reports/ZERO_INFLATION_TABPFN.md` (2026-09-17, issue #159 question 1): TabPFN `v3.5_default` vs Poisson/ZIP/ZINB GLMs on synthetic zero-inflated NB2 counts (π₀ 0→0.95, α = 1/2/4, 10 seeds); mean prediction matches the purpose-built GLMs; continuity-corrected NLL at most +0.053 nats behind the correctly-specified ZINB-GLM at any setting (ZIP-GLM +2.2 to +8.0); zero-rate error within ±0.025; no non-finite densities; log1p target transform gives no gain. The first α=1.0 run silently used v3 (the API's unpinned default moved to v3.5 on 2026-09-15) and was re-run pinned to v3.5. New `scripts/eval/zero_inflation/`, notebook `09_zero_inflation_dispersion.ipynb`, figures `outputs/current/figures/zero_inflation_*.png`; `statsmodels` added to requirements
