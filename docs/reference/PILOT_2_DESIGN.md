...
> see *What the WIDER testing still needs built* in `docs/current/FINETUNING_PREREQUISITES.md`. The probe is separate and
> needs none of it.
...
least-reliable category of guarantee. **PR-10** (in `docs/current/FINETUNING_PREREQUISITES.md`) adds the mirror assertion: context and
...
**Full explanation, written for a non-specialist reader: `docs/current/FINETUNING_COST_AND_CONTROLS.md`.** In brief:
...
> Inputs to these criteria are computed per `docs/current/FINETUNING_STATISTICAL_ANALYSIS_PLAN.md`.
...
> **How the inputs to this rule are computed is fixed in `docs/current/FINETUNING_STATISTICAL_ANALYSIS_PLAN.md`**
...
**Rejected:** the `R3 gate` in `docs/current/FINETUNING_EXPERIMENT_REFERENCE.md` — its second criterion (B > E)
...
Stage 2 multiplies Stage 1's per-target cost by the number of targets and pool policies. At P1a
   **modelled, and now consistent with the cost page:** the transfer experiment is ~$20 at 15
   repeats at sample scale (`docs/current/FINETUNING_COST_AND_CONTROLS.md` derives it from the measured arm
...
`docs/current/FINETUNING_DECISION_LOG.md`**, which is the document the review works through. Three of them
