...
| 2026-09-18 | **Spike run 1 complete** in the entry repo (`12e7f79`, `results/spike/FINDINGS.md`). The reframing works end to end on CPU with no API; `output_type="full"` returns a real distribution; empirical coverage 55/82/91/91 against nominal 50/75/90/95; and the shuffled-target placebo is 52× worse, so the harness is not leaking. But the **recursive link-ratio arm is not competitive on the point estimate** — median error ties the like-for-like Chain Ladder arm (14.9% vs 16.1%) while the mean is eight times worse (+99.3% vs +12.9%), because multiplying predicted ratios compounds upward bias and blows up on the smallest triangles. Next, in order: predict the ratio *relative to* the Chain Ladder factor, direct cumulative prediction, and a minimum-evidence rule | Recorded |
