...
> **Model versions — this page now spans two.** The canonical classification results are **TabPFN v3** (`v3_default`, client 0.3.3, Aug 2026). The four regression/count datasets — `freMTPL2freq`, `spanish_motor_freq`, `bemtl97_amount`, `spanish_motor_severity` — were re-tested on **v3.5** (`v3.5_default`, client 0.6.0) on 2026-09-16 and improved on every one (master report §16). `eudirectlapse` was re-tested on **both versions** on 2026-09-17, and its published v3 loss **did not reproduce** (§17). The six canonical classification datasets have **not** been re-tested on v3.5, so the risk-ranking verdict below remains a v3 statement. See `docs/MODEL_VERSIONS.md`.
...
- The `eudirectlapse` lapse task was disclosed as the exception to the risk-ranking story: a genuine classification loss (TabPFN AUC 0.6101 vs best GLM 0.6260 — attributed to additive-lapse structure the model doesn't capture). **This did not reproduce (§17, 2026-09-17).** On the same test folds as the published benchmark, run in the frontier harness, TabPFN v3 scores AUC 0.6331 and v3.5 0.6367, against 0.6271 for a fairly encoded (one-hot) logistic regression. v3 wins on ranking and ties on calibration; v3.5 wins on all five metrics. The August number came from TabPFN scoring 0.023 AUC lower inside the TabArena harness, for a reason not yet isolated — so treat `eudirectlapse` as a narrow TabPFN win, not a loss, and **do not cite it as evidence about additive structure.**
