...
- The `eudirectlapse` lapse task was disclosed as the exception to the risk-ranking story: a genuine classification loss (TabPFN AUC 0.6101 vs best GLM 0.6260 — attributed to additive-lapse structure the model doesn't capture). **This did not reproduce (§17, 2026-09-17).** On the identical folds in the frontier harness, TabPFN v3 scores AUC 0.6331 and v3.5 0.6367, against 0.6271 for a fairly encoded (one-hot) logistic regression. v3 wins on ranking and ties on calibration; v3.5 wins on all five metrics. The August number came from TabPFN scoring 0.023 AUC lower inside the TabArena harness, for a reason not yet isolated — so treat `eudirectlapse` as a narrow TabPFN win, not a loss, and **do not cite it as evidence about additive structure.**
